...
class ProfessionalService extends \App\PageBuilder\PageBuilderBase
{
    ...
    public function frontend_render() : string
    {
        ...
        $categories = Category::WhereHas('services')->select('id','name','image')
        ->where('status','1')
        ->take(6)
        ->inRandomOrder()
        ->get();
        ...
        foreach ($categories as $category){
            ...
            $image =  render_image_markup_by_attachment_id($category->image,'','','thumb');
            ...
            <div class="single-professional-item wow fadeInUp" data-wow-delay=".2s">
                <div class="single-professional">
                    <a href="{$route}/{$category}" class="professional-thumb">
                        {$image}
                    </a>
                    <div class="professional-contents">
                        <h6 class="professional-title"> <a href="{$route}/{$category}"> {$category} </a> </h6>
                    </div>
                </div>
            </div>
