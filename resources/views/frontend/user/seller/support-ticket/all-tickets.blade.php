            ...
            <div class="dashboard-contents-wrapper">
                ...
                <div class="dashboard-right">
                    ...
                    <div class="btn-wrapper margin-top-50 text-right">
                        <a href="{{ route('seller.orders') }}" class="cmn-btn btn-bg-1"> {{__('Create Ticket For A Order' )}}</a>
                    </div>
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $data->title }}</td>
                                        <td>
                                            @if($data->priority=='low')<span class="btn btn-primary btn-sm btn-bg-1">{{ ucfirst($data->priority) }}</span>@endif
                                            @if($data->priority=='high')<span class="btn btn-warning btn-sm btn-bg-1">{{ ucfirst($data->priority) }}</span>@endif
                                            @if($data->priority=='medium')<span class="btn btn-info btn-sm btn-bg-1">{{ ucfirst($data->priority) }}</span>@endif
                                            @if($data->priority=='urgent')<span class="btn btn-danger btn-sm btn-bg-1">{{ ucfirst($data->priority) }}</span>@endif
                                        </td>
                                        <td>
                                            @if($data->status=='open')
                                            <span class="btn btn-primary btn-sm btn-bg-1">{{ ucfirst($data->status) }}</span>
                                            ...
                                            <span class="btn btn-danger btn-sm btn-bg-1">{{ ucfirst($data->status) }}</span>
                                            <a href="{{ route('seller.support.ticket.status.change', $data->id) }}">
                                                <span class="icon dash-edit-icon-two eye-icon"><i class="las la-edit"></i></span>
                                            </a>
                                            @endif
